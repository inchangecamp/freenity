...
exports.index = async (req, res) => { 
    ...
    Payments.find(filter)
        ...
        .exec().then(async (messages) => {
            ...
            log.success('Retrieved all {} Payments', messages.length)
            res.json({ data: messages })
        })
        .catch(err => {
            log.error(err, 'Could not retrieve Payments: {}', err.message) 
            res.json({ data: [] })
        })
}
...
exports.store = async (req, res) => {
    ...
    if (data._id) {
        ...
        Payments.findOneAndUpdate({_id: data._id}, {$set: data}, {new: true}).then(async (message) => {
            message = JSON.parse(JSON.stringify(message))

            log.success('Updated Payments: {}', message._id)
            return res.status(200).json({ data: message, method: 'update' })
        })
    } else {
        data.user_id = req.user._id
        let message = new Payments(req.body)
        ...
        await message.save().then(async (message) => {
            ...
            log.success('Created Payments: {}', message._id)
			
            return res.status(200).json({ data: message, method: 'create' })
        })
        .catch(err => {
            log.error(err, 'Error creating Payments: {}', message._id)
...
exports.show = async (req, res) => {
    Payments.findById(req.params.id).then(async (message) => {
        message = JSON.parse(JSON.stringify(message))
...
exports.delete = async (req, res) => {
    Payments.findById(req.params.id).remove().exec()
    res.json({
        message: 'Deleted'
    })
}
