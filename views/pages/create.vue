...
export default {
    ...
    methods: {
        async removeMessage(message) {
            ...
            await axios.delete('/create/' + message._id).then(response => {
                app.$refs.messageFormComponent.defaultState()
                app.messages = app.messages.filter(item => {
                    return item._id !== message._id
                })

                window.ws.send(JSON.stringify({event: 'delete_create', data: message}));
            })
        },
        createMessage(message) {
            ...
            window.ws.send(JSON.stringify({event: 'create_create', data: message}));
