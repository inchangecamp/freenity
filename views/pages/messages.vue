...
export default {
    ...
    methods: {
        ...
        editMessage(message) {
			...
			//eval("message.comment_"+langs+" = message.comment_"+langs+".replace(/<br>/g, '\n');")
        ...
        async fetchMessages(offset, $state) {
			...
			if (params['cat']) var categories=params['cat']
			else var categories=0

			await axios.get('/messages', { params: { offset: offset || 0, categories: categories  } })
                .then(async response => {
