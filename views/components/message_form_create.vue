<template>
    ...
    <div class="position-sticky w-100" style="bottom: 0;">
        ...
        <div class="card" v-if="$auth.check(['creator','editor'])">
						...
						<div class="d-flex flex-row">
							...
							<div class="d-flex flex-column flex-grow-1 ml-3">					
								 ...
								 <textarea class="form-control flex-grow-1" rows="1" id="url" v-model="fields.url" 
									...
									@delete="debounceScrapingMeta;show_close();"	placeholder="link" style="border: 0px;"></textarea>
            ...
            <div class="card-body d-flex flex-column" style="box-shadow:none;border-top:1px solid #D0D0D0;border-bottom:1px solid #D0D0D0;">
                ...
                <div class="d-flex flex-row">
                    ...
                    <div class="d-flex flex-column flex-grow-1 ml-3">					
                        ...
                        <resize-textarea>
                            <textarea class="form-control flex-grow-1" rows="1" id="comment" v-model="fields.comment"
								@paste="show_close();" 
								@keypress="show_close();" 
								@keyup="show_close();" 
...
export default {
    ...
    methods: {
		...
		go_shop() {
			...
			window.location.href='/shop?lang='+langs;
            ...
            if (app.passedData) {
			   ...
			   app.fields = { url: app.data.url, comment: eval("app.data.comment_"+langs), special: eval("app.data.special_post_"+langs) }
				...
				.then(async response => {
					...
					$('.field_categories').show();
					$('.field_categories').html(eval("response.data.data.title_"+langs)+'<i onclick="$(\'#categories\').val(\'\');$(\'.field_categories\').html(\'\')" class="mdi mdi-close text-secondary" style="cursor: pointer;font-size: 10px;"></i>');
